...
create_test_jsp() {
        IP_ADDRESS="$(/sbin/ip addr show eth1 | grep 'inet' | grep -v inet6 | tr -s ' ' | cut -d ' ' -f 3 | cut -d '/' -f 1)"
        LAST_NUMBER_OF_IP_ADDRESS="$(/sbin/ip addr show eth1 | grep 'inet' | grep -v inet6 | tr -s ' ' | cut -d ' ' -f 3 | cut -d '/' -f 1 | cut -d '.' -f 4)"
        COLOR="black"
