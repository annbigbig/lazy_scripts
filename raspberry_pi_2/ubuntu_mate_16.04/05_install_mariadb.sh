...
install_cmake() {
	...
	chown -R root:root ./cmake-3.5.2
	cd ./cmake-3.5.2
	./bootstrap
	make
	ln -s /usr/local/src/cmake-3.5.2/bin/cmake /usr/local/sbin/cmake
	ln -s /usr/local/src/cmake-3.5.2/bin/ctest /usr/local/sbin/ctest
	ln -s /usr/local/src/cmake-3.5.2/bin/cpack /usr/local/sbin/cpack
	rm -rf /usr/local/src/cmake-3.5.2.tar.gz
	echo "done."
}
...
install_mariadb() {
	...
	cd /usr/local/src
	wget https://downloads.mariadb.org/interstitial/mariadb-10.1.13/source/mariadb-10.1.13.tar.gz
	tar -zxvf ./mariadb-10.1.13.tar.gz
	chown -R root:root ./mariadb-10.1.13
	cd ./mariadb-10.1.13
	...
	cmake -DCMAKE_BUILD_TYPE=Release \
		...
		-DINSTALL_DOCDIR=share/doc/mariadb-10.1.13 \
		-DINSTALL_DOCREADMEDIR=share/doc/mariadb-10.1.13 \
	...
	make test
	make install
	ln -s /usr/local/mariadb-10.1.13 /usr/local/mariadb
	echo "done."
}
