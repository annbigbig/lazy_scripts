...
install_tomcat() {
	...
	chown -R root:root ./apache-tomcat-8.5.56
	chmod -R a+r ./apache-tomcat-8.5.56
        find /usr/local/apache-tomcat-8.5.56 -type d -exec chmod a+rx {} \;
	rm -rf /usr/local/tomcat
	ln -s /usr/local/apache-tomcat-8.5.56 /usr/local/tomcat
	...
	cd /usr/local/tomcat/conf/
        ...
        cat > /usr/local/apache-tomcat-8.5.56/conf/tomcat-users.xml << "EOF"
...
EOF
        sed -i -- "s|TOMCAT_ADMIN_USERNAME|$TOMCAT_ADMIN_USERNAME|g" /usr/local/apache-tomcat-8.5.56/conf/tomcat-users.xml
        sed -i -- "s|TOMCAT_ADMIN_PASSWORD|$TOMCAT_ADMIN_PASSWORD|g" /usr/local/apache-tomcat-8.5.56/conf/tomcat-users.xml

        echo -e "configure JNDI DataSource"
        cd /usr/local/tomcat/conf/
        cp server.xml server.xml.default
        rm -rf server.xml
        cat > /usr/local/apache-tomcat-8.5.56/conf/server.xml << "EOF"
<?xml version="1.0" encoding="UTF-8"?>
...
EOF
        sed -i -- "s|TOMCAT_JNDI_RESOURCE_NAME|$TOMCAT_JNDI_RESOURCE_NAME|g" /usr/local/apache-tomcat-8.5.56/conf/server.xml
        sed -i -- "s|TOMCAT_JNDI_USERNAME|$TOMCAT_JNDI_USERNAME|g" /usr/local/apache-tomcat-8.5.56/conf/server.xml
        sed -i -- "s|TOMCAT_JNDI_PASSWORD|$TOMCAT_JNDI_PASSWORD|g" /usr/local/apache-tomcat-8.5.56/conf/server.xml
        sed -i -- "s|TOMCAT_JNDI_URL|$TOMCAT_JNDI_URL|g" /usr/local/apache-tomcat-8.5.56/conf/server.xml
...
EOF
        sed -i -- "s|TOMCAT_JNDI_RESOURCE_NAME|$TOMCAT_JNDI_RESOURCE_NAME|g" /usr/local/apache-tomcat-8.5.56/conf/context.xml
        sed -i -- "s|TOMCAT_MEMCACHED_NODES|$TOMCAT_MEMCACHED_NODES|g" /usr/local/apache-tomcat-8.5.56/conf/context.xml


        # unlock host-manager and manager that only be accessed by 127.0.0.1
        rm -rf /usr/local/apache-tomcat-8.5.56/webapps/manager/META-INF/context.xml
        rm -rf /usr/local/apache-tomcat-8.5.56/webapps/host-manager/MATA-INF/context.xml

        cat > /usr/local/apache-tomcat-8.5.56/webapps/manager/META-INF/context.xml << "EOF"
<?xml version="1.0" encoding="UTF-8"?>
<!--
...
EOF
        cp /usr/local/apache-tomcat-8.5.56/webapps/manager/META-INF/context.xml /usr/local/apache-tomcat-8.5.56/webapps/host-manager/META-INF/context.xml
        chmod 644 /usr/local/apache-tomcat-8.5.56/webapps/manager/META-INF/context.xml
        chmod 644 /usr/local/apache-tomcat-8.5.56/webapps/host-manager/META-INF/context.xml

        # download jar file 'slf4j-api-1.7.25.jar' and place it in $CATALINA_HOME/lib
        wget -O /usr/local/apache-tomcat-8.5.56/lib/slf4j-api-1.7.25.jar https://repo.maven.apache.org/maven2/org/slf4j/slf4j-api/1.7.25/slf4j-api-1.7.25.jar

        # download jar files for JNDI resource settings
        cd /usr/local/apache-tomcat-8.5.56/lib/
	...
	chown root:root /usr/local/apache-tomcat-8.5.56/lib/*.jar
	chmod 644 /usr/local/apache-tomcat-8.5.56/lib/*.jar
...
install_gradle() {
	...
	cd /usr/local
        ...
        wget https://services.gradle.org/distributions/gradle-6.5-all.zip.sha256
        SHA256SUM_SHOULD_BE="$(/bin/cat ./gradle-6.5-all.zip.sha256 | cut -d ' ' -f 1)"
        SHA256SUM_COMPUTED="$(/usr/bin/sha256sum ./gradle-6.5-all.zip | cut -d ' ' -f 1)"
        [ "$SHA256SUM_SHOULD_BE" == "$SHA256SUM_COMPUTED" ] && echo "gradle sha256sum matched." || exit 2
	unzip ./gradle-6.5-all.zip
	chown -R root:root ./gradle-6.5
	rm -rf /usr/local/gradle
	ln -s /usr/local/gradle-6.5 /usr/local/gradle
...
install_spring_boot_cli() {
	...
	wget https://repo.spring.io/release/org/springframework/boot/spring-boot-cli/2.3.1.RELEASE/spring-boot-cli-2.3.1.RELEASE-bin.tar.gz.md5
        MD5SUM_SHOULD_BE="$(/bin/cat ./spring-boot-cli-2.3.1.RELEASE-bin.tar.gz.md5 | cut -d ' ' -f 1)"
        MD5SUM_COMPUTED="$(/usr/bin/md5sum ./spring-boot-cli-2.3.1.RELEASE-bin.tar.gz | cut -d ' ' -f 1)"
        [ "$MD5SUM_SHOULD_BE" == "$MD5SUM_COMPUTED" ] && echo "spring-boot-cli md5sum matched." || exit 2

        tar -zxvf ./spring-boot-cli-2.3.1.RELEASE-bin.tar.gz
	chown -R root:root ./spring-2.3.1.RELEASE
	rm -rf /usr/local/spring-boot-cli
	ln -s /usr/local/spring-2.3.1.RELEASE /usr/local/spring-boot-cli
	rm -rf ./spring-boot-cli-2.3.1.RELEASE-bin.tar.gz*
}
...
install_eclipse_ee() {
	...
	cd /usr/local
	rm -rf ./eclipse
	wget https://www.eclipse.org/downloads/download.php?file=/technology/epp/downloads/release/2019-12/R/eclipse-jee-2019-12-R-linux-gtk-x86_64.tar.gz\&r=1 -O eclipse-jee-2019-12-R-linux-gtk-x86_64.tar.gz

	wget http://download.eclipse.org/technology/epp/downloads/release/2019-12/R/eclipse-jee-2019-12-R-linux-gtk-x86_64.tar.gz.md5
	MD5SUM_SHOULD_BE="$(/bin/cat ./eclipse-jee-2019-12-R-linux-gtk-x86_64.tar.gz.md5 | cut -d ' ' -f 1)"
        MD5SUM_COMPUTED="$(/usr/bin/md5sum ./eclipse-jee-2019-12-R-linux-gtk-x86_64.tar.gz | cut -d ' ' -f 1)"
        [ "$MD5SUM_SHOULD_BE" == "$MD5SUM_COMPUTED" ] && echo "Eclipse EE md5sum matched." || exit 2
	
	tar -zxvf ./eclipse-jee-2019-12-R-linux-gtk-x86_64.tar.gz
	chown -R root:root ./eclipse

        # find the user whose UID is 1000
        YOUR_USERNAME="$(/bin/cat /etc/passwd | grep 1000 | head -1 | cut -d ':' -f 1)"

	rm -rf /home/$YOUR_USERNAME/桌面/eclipse-EE-2019-12-R
	ln -s /usr/local/eclipse/eclipse /home/$YOUR_USERNAME/桌面/eclipse-EE-2019-12-R
	rm -rf ./eclipse-jee-2019-12-R-linux-gtk-x86_64.tar.gz
...
register_tomcat_as_systemd_service() {
	...
	chown -R tomcat:tomcat /usr/local/apache-tomcat-8.5.56
