...
install_spring_boot_cli() {
	...
	cd /usr/local
        ...
        tar -zxvf ./spring-boot-cli-1.3.3.RELEASE-bin.tar.gz
	chown -R root:root ./spring-1.3.3.RELEASE
	ln -s /usr/local/spring-1.3.3.RELEASE /usr/local/spring-boot-cli
	rm -rf ./spring-boot-cli-1.3.3.RELEASE-bin.tar.gz
}
