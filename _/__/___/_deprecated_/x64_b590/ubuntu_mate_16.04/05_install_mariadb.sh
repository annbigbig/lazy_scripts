...
install_mariadb() {
	...
	cd /usr/local/src
	wget https://downloads.mariadb.org/interstitial/mariadb-10.1.14/source/mariadb-10.1.14.tar.gz
	tar -zxvf ./mariadb-10.1.14.tar.gz
	chown -R root:root ./mariadb-10.1.14
	cd ./mariadb-10.1.14
	sed -i "s@data/test@\${INSTALL_MYSQLTESTDIR}@g" sql/CMakeLists.txt
	mkdir build
	cd build
	cmake -DCMAKE_BUILD_TYPE=Release \
		-DCMAKE_INSTALL_PREFIX=/usr/local/mariadb-10.1.14 \
	...
	make test
	make install
	ln -s /usr/local/mariadb-10.1.14 /usr/local/mariadb
	echo "done."
}
