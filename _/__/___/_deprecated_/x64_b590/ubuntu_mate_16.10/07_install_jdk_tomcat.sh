...
install_tomcat() {
	...
	cd /usr/local
	wget http://apache.stu.edu.tw/tomcat/tomcat-8/v8.5.8/bin/apache-tomcat-8.5.8.tar.gz
	tar -zxvf ./apache-tomcat-8.5.8.tar.gz
	chown -R root:root ./apache-tomcat-8.5.8
	chmod -R a+r ./apache-tomcat-8.5.8/conf
...
install_gradle() {
	...
	cd /usr/local
	wget https://services.gradle.org/distributions/gradle-3.2.1-all.zip
	unzip ./gradle-3.2.1-all.zip
	chown -R root:root ./gradle-3.2.1
	rm -rf /usr/local/gradle
	ln -s /usr/local/gradle-3.2.1 /usr/local/gradle
	rm -rf ./gradle-3.2.1-all.zip
}

install_spring_boot_cli() {
	echo -e "ready to install spring boot cli\n"
	cd /usr/local
        wget http://repo.spring.io/release/org/springframework/boot/spring-boot-cli/1.4.2.RELEASE/spring-boot-cli-1.4.2.RELEASE-bin.tar.gz
        tar -zxvf ./spring-boot-cli-1.4.2.RELEASE-bin.tar.gz
	chown -R root:root ./spring-1.4.2.RELEASE
	rm -rf /usr/local/spring-boot-cli
	ln -s /usr/local/spring-1.4.2.RELEASE /usr/local/spring-boot-cli
	rm -rf ./spring-boot-cli-1.4.2.RELEASE-bin.tar.gz
}
...
install_eclipse_ee() {
	...
	cd /usr/local
	rm -rf ./eclipse
	wget http://www.eclipse.org/downloads/download.php?file=/technology/epp/downloads/release/oxygen/M3/eclipse-jee-oxygen-M3-linux-gtk-x86_64.tar.gz\&r=1 -O eclipse-jee-oxygen-M3-linux-gtk-x86_64.tar.gz

	tar -zxvf ./eclipse-jee-oxygen-M3-linux-gtk-x86_64.tar.gz
	chown -R root:root ./eclipse
	rm -rf /home/$YOUR_USERNAME/桌面/eclipse-EE-oxygen-M3
	ln -s /usr/local/eclipse/eclipse /home/$YOUR_USERNAME/桌面/eclipse-EE-oxygen-M3
	rm -rf ./eclipse-jee-oxygen-M3-linux-gtk-x86_64.tar.gz
}
...
register_tomcat_as_systemd_service() {
	...
	chown -R tomcat:tomcat /usr/local/apache-tomcat-8.5.8
