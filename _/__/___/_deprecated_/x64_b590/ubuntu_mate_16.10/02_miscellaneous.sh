#!/bin/bash
# This script will perform lots of work for fine tune Ubuntu 16.10 
# before you run this script , please specify some parameters here:
#
LAN1="10.1.1.0/24" # The local network(1) that you allow packets come in from there
LAN2="10.2.2.0/24" # The local network(2) that you allow packets come in from there
VPN1="10.8.0.0/24" # The VPN network(1) that you allow packets come in from there
VPN2="10.5.3.0/24" # The VPN network(2) that you allow packets come in from there
#####################
...
loopback=127.0.0.1
local="\$(/sbin/ifconfig wlan0 | grep netmask | cut -d ' ' -f 10)"
#local="\$(/sbin/ifconfig eth0 | grep netmask | cut -d ' ' -f 10)"
#local=10.1.1.170
lan1=$LAN1
lan2=$LAN2
vpn1=$VPN1
...
#\$iptables -t filter -A INPUT -i wlan0 -s \$lan1 -d \$local -p all -j ACCEPT
#\$iptables -t filter -A INPUT -i wlan0 -s \$lan2 -d \$local -p all -j ACCEPT
#\$iptables -t filter -A INPUT -i wlan0 -s \$vpn1 -d \$local -p all -j ACCEPT
#\$iptables -t filter -A INPUT -i wlan0 -s \$vpn2 -d \$local -p all -j ACCEPT
#\$iptables -t filter -A INPUT -i eth0 -s \$local -d \$local -p all -j ACCEPT
#\$iptables -t filter -A INPUT -i eth0 -s \$lan1 -d \$local -p all -j ACCEPT
#\$iptables -t filter -A INPUT -i eth0 -s \$lan2 -d \$local -p all -j ACCEPT
#\$iptables -t filter -A INPUT -i eth0 -s \$vpn1 -d \$local -p all -j ACCEPT
#\$iptables -t filter -A INPUT -i eth0 -s \$vpn2 -d \$local -p all -j ACCEPT
\$iptables -t filter -A INPUT -s \$local -d \$local -p all -j ACCEPT
\$iptables -t filter -A INPUT -s \$lan1 -d \$local -p all -j ACCEPT
\$iptables -t filter -A INPUT -s \$lan2 -d \$local -p all -j ACCEPT
...
\$iptables -t filter -A INPUT -s \$lan1 -p tcp --dport 36000 --syn -m state --state NEW -j ACCEPT
\$iptables -t filter -A INPUT -s \$lan2 -p tcp --dport 36000 --syn -m state --state NEW -j ACCEPT
\$iptables -t filter -A INPUT -s \$vpn1 -p tcp --dport 36000 --syn -m state --state NEW -j ACCEPT
\$iptables -t filter -A INPUT -s \$vpn2 -p tcp --dport 36000 --syn -m state --state NEW -j ACCEPT
...
\$iptables -t filter -A INPUT -s \$lan1 -d \$local -p icmp -j ACCEPT
\$iptables -t filter -A INPUT -s \$lan2 -d \$local -p icmp -j ACCEPT
