...
fix_network_interfaces_name(){
	ETH0_MAC_ADDRESS=$(ifconfig |grep enp2s0 | cut -d ' ' -f 9)
	ETH1_MAC_ADDRESS=$(ifconfig |grep enp3s0 | cut -d ' ' -f 9)
	WLAN0_MAC_ADDRESS=$(ifconfig |grep wlp4s0 | cut -d ' ' -f 9)
	...
	if [ $HOW_MANY_LINES_IN_NETWORK_RULES_FILE -eq 3 ]; then
